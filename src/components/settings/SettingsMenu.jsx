...
import { BellRing, CloudSun, Loader2, Palette, Settings2 } from 'lucide-react';
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-2xl">
        <DialogHeader>
          ...
          <DialogDescription>
            Tutte le opzioni sono ora organizzate in sezioni: Generali, Promemoria e Meteo.
          </DialogDescription>
        </DialogHeader>

        <Tabs defaultValue="general" className="w-full">
          <TabsList className="grid w-full grid-cols-3">
            <TabsTrigger value="general" className="gap-1"><Palette className="h-3.5 w-3.5" /> Generali</TabsTrigger>
            <TabsTrigger value="reminders" className="gap-1"><BellRing className="h-3.5 w-3.5" /> Promemoria</TabsTrigger>
            <TabsTrigger value="weather" className="gap-1"><CloudSun className="h-3.5 w-3.5" /> Meteo</TabsTrigger>
          </TabsList>

          <TabsContent value="general" className="max-h-[60vh] overflow-y-auto space-y-4 pr-1">
          ...
          <TabsContent value="reminders" className="max-h-[60vh] overflow-y-auto space-y-4 pr-1">
            <div className="space-y-2 rounded-lg border p-3">
              <Label htmlFor="reminder-email">Email promemoria</Label>
              <Input
                id="reminder-email"
                type="email"
              ...
              {emailError ? (
                ...
                <p className="text-xs text-muted-foreground">Se vuota, l'email non viene inviata</p>
              )}
              <p className="text-xs text-muted-foreground">
                Stato email: {emailValidity === 'valid' ? 'valida' : emailValidity === 'invalid' ? 'non valida' : 'non impostata'}
              </p>
              <Button type="button" variant="outline" size="sm" onClick={sendTestEmail} disabled={isSendingTest}>
            ...
            <div className="space-y-3 rounded-lg border p-3">
              ...
              <div className="flex items-center justify-between">
                ...
                <Switch checked={reminderChannels.email} onCheckedChange={(checked) => updateReminderChannel('email', checked)} />
              </div>
              <div className="flex items-center justify-between">
                <p className="text-sm">Notifiche</p>
                <Switch checked={reminderChannels.notification} onCheckedChange={(checked) => updateReminderChannel('notification', checked)} />
              </div>
            ...
            <div className="space-y-3 rounded-lg border p-3">
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  ...
                  <Input type="time" value={settings.notificationQuietHours?.start ?? '22:00'} onChange={(event) => setQuietHoursField('start', event.target.value)} />
                </div>
                <div>
                  <Label className="text-xs">a</Label>
                  <Input type="time" value={settings.notificationQuietHours?.end ?? '07:00'} onChange={(event) => setQuietHoursField('end', event.target.value)} />
                </div>
              </div>
              <div className="flex items-center justify-between">
                <p className="text-sm">Abilita orari silenziosi</p>
                <Switch checked={settings.notificationQuietHours?.enabled ?? false} onCheckedChange={(checked) => setQuietHoursField('enabled', checked)} />
              </div>
              <div className="flex items-center justify-between">
                <p className="text-sm">Solo priorità alta in orari silenziosi</p>
                <Switch checked={settings.notificationHighPriorityOnlyInQuietHours} onCheckedChange={(checked) => updateSetting('notificationHighPriorityOnlyInQuietHours', checked)} />
              </div>
          ...
          <TabsContent value="weather" className="max-h-[60vh] overflow-y-auto space-y-4 pr-1">
            <div className="space-y-3 rounded-lg border p-3">
              ...
              <div className="flex items-center justify-between">
                ...
                <Switch checked={settings.weatherEnabled} onCheckedChange={(checked) => updateSetting('weatherEnabled', checked)} />
              </div>
              <div className="flex items-center justify-between">
